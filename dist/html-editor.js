console.log('[HTML Editor] Script loaded, preparing Monaco initialization');
...
(function() {
    ...
    console.log('[HTML Editor] Temporarily hiding AMD for Monaco loader');
    ...
    loaderScript.onload = () => {
        console.log('[HTML Editor] Monaco loader loaded, restoring page AMD');
        ...
        console.log('[HTML Editor] Page AMD restored, Monaco require stored separately');
    };
    loaderScript.onerror = () => {
        console.error('[HTML Editor] Failed to load Monaco loader');
...
const editorState = {
    ...
    tail: { active: false, editor: null, content: '', label: 'Page HTML Tail', isDirty: false }
};
...
function scanDOMForAutocomplete() {
    ...
    const allElements = document.querySelectorAll('*:not(#html-editor-overlay *):not(#html-editor-app *)');
...
function checkViewportWidth() {
    ...
    // Check if we're in overlay mode (inside #html-editor-overlay)
    ...
    if (isInOverlay) {
        ...
        const overlay = document.getElementById('html-editor-overlay');
        if (overlay) {
...
function parseHTML(html) {
    ...
    Object.entries(textareas).forEach(([name, key]) => {
        ...
        if (textarea) {
            ...
            console.log(`[parseHTML] Found ${key} HTML: ${cssData.css[key].length} characters`);
        } else {
...
function buildMultipartBody(cssData) {
    ...
    const fields = {
        ...
        'html_template_tail': cssData.html_template_tail
    };
...
async function saveCSS() {
    ...
    try {
        ...
        console.log('[saveCSS] HTML data collected from state:', {
            head: cssData.html_template_head?.length || 0,
            tail: cssData.html_template_tail?.length || 0
        });
